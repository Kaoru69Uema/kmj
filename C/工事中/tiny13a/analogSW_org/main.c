//
// 令和4年10月27日（2022年10月27日）ホームページからコピー
//
...
volatile unsigned char t_flag=0;        // 1ms時間調整フラグ
         ...
         unsigned int  aswin =0;        // ADC2（PB4）値
...
int main(void)
{
    ...
    PORTB = 0b00010000;
    {
        TCCR0A=0b00000010;  // ﾀｲﾏ/ｶｳﾝﾀ制御ﾚｼﾞｽﾀA
        //       |||||||+---WGM00  波形生成種別     比較一致ﾀｲﾏ/ｶｳﾝﾀ解除(CTC)動作
        //       ||||||+----WGM01  波形生成種別     (TCCR0B:WGM02),WGM01,WGM00=0,1,0
        //       |||||+----- -     予約
        //       ||||+------ -     予約
        //       |||+-------COM0B0 比較B出力選択
        //       ||+--------COM0B1 比較B出力選択
        //       |+---------COM0A0 比較A出力選択
        //       +----------COM0A1 比較A出力選択
        TCCR0B=0b00001010;  // ﾀｲﾏ/ｶｳﾝﾀ制御ﾚｼﾞｽﾀB
        //       |||||+++---CS02,CS01,CS00   入力ｸﾛｯｸ選択 クロックを８分周
        //       ||||+------WGM02  波形生成種別
        //       ||++------- -     予約
        //       |+---------FOC0B  OC0B強制変更
        //       +----------FOC0A  OC0A強制変更
        OCR0A=1200000/8/1000-1;  // ﾀｲﾏ/ｶｳﾝﾀ 比較Aﾚｼﾞｽﾀ  1000Hzで割り込み
        TIMSK0=0b00000100;  // ﾀｲﾏ/ｶｳﾝﾀ割り込みﾏｽｸ ﾚｼﾞｽ
        //       |||||||+--- - 予約
        //       ||||||+---- TOIE0:ﾀｲﾏ/ｶｳﾝﾀ溢れ割り込み許可
        //       |||||+----- OCIE0A:ﾀｲﾏ/ｶｳﾝﾀ比較A割り込み許可
        //       ||||+------ OCIE0B:ﾀｲﾏ/ｶｳﾝﾀ比較B割り込み許可
        //       ++++------- - 予約
        TCNT0=0; // タイマ／カウンタ
    }
    ...
    //         |||+---------ADIF A/D変換完了割り込み要求ﾌﾗｸﾞ A/D変換が完了で１
    //         ||+----------ADATE A/D変換自動起動 : 不許可
    //         |+-----------ADSC １でA/D変換開始
    //         +------------ADEN A/D許可
    ...
    while(1)
    {
        ...
        else if( tmp1 ==  2 ) tmp1 =   2;    //////////////////////
        else if( tmp1 ==  3 ) tmp1 =   3;    //////////////////////
        else if( tmp1 ==  4 ) tmp1 =   7;    //////////////////////
        else if( tmp1 ==  5 ) tmp1 =   6;    //////////////////////
        else if( tmp1 ==  6 ) tmp1 =   5;    //////////////////////
        else if( tmp1 ==  7 ) tmp1 =   4;    //////////////////////   （ハードの失敗をソフトで調整する）
        else if( tmp1 ==  8 ) tmp1 =   8;    //////////////////////
        else if( tmp1 ==  9 ) tmp1 =   9;    //////////////////////
        else if( tmp1 == 10 ) tmp1 =  10;    //////////////////////
        else if( tmp1 == 11 ) tmp1 =  11;    //////////////////////
        else if( tmp1 == 12 ) tmp1 =  15;    //////////////////////
        else if( tmp1 == 13 ) tmp1 =  14;    //////////////////////
        else if( tmp1 == 14 ) tmp1 =  13;    //////////////////////
        else if( tmp1 == 15 ) tmp1 =  12;    //////////////////////
        ///////////////////////////////////////////////////////////
...
void asw_main( void )
{
        ADCSRA |= 1<<ADIF;                      //変換完了フラグをリセット
                                                //書き込む事でリセットできる。変換前に呼び出す事
        ADCSRA |= 1<<ADSC;                      //ADSC(A/D変換開始)

        while( (ADCSRA & (1<<ADIF) )==0 ) ;     //変換完了フラグが1になるまで待機
